...
----
The Cordova Plugin is instantiated when it is refernced from the Javascipt webview app. The `pluginInitiaized()` includes the startup code.
...
. Add implementation of `queryHotels` method to *cordova-plugin-hotel-lister/src/ios/HotelLister.swift*.
+
[source,swift]
----
