...
----
The Cordova Plugin is instantiated when it is refernced from the Javascipt webview app. The `pluginInitiaized()` includes the startup code. The `exec` method marshalls requests from the Javascipt to Android Side.
